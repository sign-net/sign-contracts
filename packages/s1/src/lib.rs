...
use s_std::{create_fund_community_pool_msg, Response, SubMsg, NATIVE_DENOM};
use thiserror::Error;
...
mod tests {
    ...
    fn check_fair_burn_no_dev_rewards() {
        ...
        let burn_msg = SubMsg::new(BankMsg::Burn {
            amount: coins(500, "usign".to_string()),
        });
    ...
    fn check_fair_burn_with_dev_rewards() {
        ...
        fair_burn(1000u128, Some(Addr::unchecked("ben")), &mut res);
        let bank_msg = SubMsg::new(BankMsg::Send {
            to_address: "ben".to_string(),
            amount: coins(100, "usign".to_string()),
        });
        let burn_msg = SubMsg::new(BankMsg::Burn {
            amount: coins(400, "usign".to_string()),
        });
